      ...
      try {
        ...
        if (userError) {
          ...
          try {
            ...
            if (prefsError) {
              ...
              if (prefsError.code !== "PGRST116") {
                ...
                if (prefsError.code === "42P01") {
                  ...
                  throw new Error(
                    "The user_preferences table does not exist. The system may need initialization."
                  );
                } else {
                  throw prefsError;
                }
            ...
            if (prefsData) {
              setPreferences({
                emailNotificationsEnabled:
                  prefsData.email_notifications_enabled ?? true,
                notifyOnStatusChange: prefsData.notify_on_status_change ?? true,
                notifyOnAssigneeChange:
                  prefsData.notify_on_assignee_change ?? true,
              ...
              if (insertError) {
                ...
                throw new Error(
                  `Failed to create default preferences: ${insertError.message}`
                );
              } else {
            ...
            setError(
              `Failed to load notification preferences: ${tableError.message}`
            );
          }
        }
      } catch (err) {
      ...
      {saving && (
        <div className="ticket-modal-loading-overlay">
          <div className="ticket-modal-loading-spinner">
            ...
            <p className="loading-text">Saving preferences...</p>
          </div>
        </div>
      )}
                        ...
                        if (prefsData) {
                          setPreferences({
                            ...
                            notifyOnStatusChange:
                              prefsData.notify_on_status_change ?? true,
                            notifyOnAssigneeChange:
                              prefsData.notify_on_assignee_change ?? true,
                            notifyOnComments:
                              prefsData.notify_on_comments ?? true,
                            notifyOnMentions:
                              prefsData.notify_on_mentions ?? true,
                            dailyDigest: prefsData.daily_digest ?? false,
                          });
                        }
                      }
        ...
        <Form>
          <Form.Group className="mb-4">
            ...
            <Form.Text className="text-muted">
              Master switch for all email notifications. Turn this off to stop
