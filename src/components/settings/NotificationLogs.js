...
const NotificationLogs = () => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        ...
        if (userError) {
          console.log(
            "No authenticated user found, using mock user for development"
          );
        }
        ...
        try {
          ...
          if (error) {
            if (error.code === "42P01") {
              ...
              throw new Error(
                "The email_notification_logs table does not exist. The system may need initialization."
              );
            } else {
              throw error;
            }
  ...
  // Get recipient emails from recipient IDs
  const getRecipientEmails = (recipientIds) => {
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    setError(
                      `Failed to initialize notification tables: ${err.message}`
                    );
                  } finally {
                    setLoading(false);
                  }
        ...
        {loading ? (
              ...
              {filteredLogs.map((log) => (
                <React.Fragment key={log.id}>
                  <tr>
                    ...
                    <td>
                      <a href={`/tickets/${log.ticket_id}`}>#{log.ticket_id}</a>
                    </td>
                    ...
                    <td>
                      {log.recipients && log.recipients.length > 0 ? (
                        <span
                          ...
                          title="Click to view recipient details"
                        >
                    ...
                    <td>
                      ...
                      {log.metadata && log.type === "mention" && (
                        <div>
                          <div className="text-muted small mb-1">
                            Mentioned: {log.metadata.mentionedUser}
                          </div>
                          <div
                            className="text-truncate"
                            style={{ maxWidth: "300px" }}
                            title={log.metadata.commentContent}
                          >
                            "{log.metadata.commentContent}"
                          </div>
                        </div>
                      )}
                    </td>
                  ...
                  {/* Expandable row for recipient details */}
                  {expandedRows.has(log.id) && (
                    <tr>
                      <td
                        colSpan="6"
                      ...
                      >
                        <div>
                          <strong className="text-muted">Recipients:</strong>
                          <div className="mt-2">
                            {getRecipientEmails(log.recipients).map(
                              (recipient, index) => (
          ...
          !error && (
            <div className="text-center py-5">
              <div className="mb-3">
                <i
                  className="bi bi-envelope-open text-muted"
                  style={{ fontSize: "3rem" }}
                ></i>
              </div>
              <p className="text-muted">No notification logs found.</p>
              <p className="text-muted small">
                Email notification logs will appear here after emails are sent.
              </p>
              <Button
                  ...
                  try {
                    ...
                    if (currentUser) {
                      await emailService.sendTicketNotification({
                        ...
                        message:
                          "This is a test notification to verify the email logging system.",
                        recipients: [
                    ...
                    setError(
                      `Failed to create test notification: ${err.message}`
                    );
                  } finally {
                    setLoading(false);
                  }
