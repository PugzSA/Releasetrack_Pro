    ...
    <div className="metadata-card">
      <div className="metadata-card-header">
        ...
        <div className="metadata-card-badges">
          <span
            className={`status-badge ${metadataItem.type
              ?.replace(/\s+/g, "-")
              .toLowerCase()}`}
          >
            {metadataItem.type}
          ...
          <span
            className={`status-badge ${metadataItem.action?.toLowerCase()}`}
          >
            {metadataItem.action}
          </span>
        </div>
        ...
        <div className="metadata-card-detail-item">
          ...
          {metadataItem.ticket_id
            ? `${getTicketInfo(metadataItem.ticket_id).id} - ${
                getTicketInfo(metadataItem.ticket_id).title
              }`
            : "None"}
        </div>
        <span className="metadata-card-divider">|</span>
        <div className="metadata-card-detail-item">
          <i className="bi bi-box"></i> Release:{" "}
          {getReleaseName(metadataItem.release_id)}
        </div>
        <span className="metadata-card-divider">|</span>
        <div className="metadata-card-detail-item">
          <i className="bi bi-calendar3"></i> Created Date:{" "}
      ...
      <div className="metadata-card-footer">
        <span className={`status-badge ${metadataItem.object?.toLowerCase()}`}>
